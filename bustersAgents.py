# bustersAgents.py͏󠄂͏️͏󠄌͏󠄎͏︈͏︂͏︁
# ----------------͏󠄂͏️͏󠄌͏󠄎͏︈͏︂͏︁
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ## DO NOT CHANGE ANYTHING IN THE CODE ABOVE THIS LINE!!!

        livingGhostsLikelyPositions = [distribution.argMax() for distribution in livingGhostPositionDistributions]
        closestGhost = min([(self.distancer.getDistance(pacmanPosition, ghostPosition), ghostPosition) for ghostPosition in livingGhostsLikelyPositions])[1]
        chosenAction = min(legal, key=lambda action: self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestGhost))
