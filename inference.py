...
class InferenceModule:
    ...
    """
    ############################################
    # Useful methods for all inference modules #͏󠄂͏️͏󠄌͏󠄎͏︈͏︂͏︁
    ############################################
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        #raiseNotDefined()͏󠄂͏️͏󠄌͏󠄎͏︈͏︂͏︁
    ...
    ######################################
    # Methods that need to be overridden #͏󠄂͏️͏󠄌͏󠄎͏︈͏︂͏︁
    ######################################
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.legalPositions:
            observationProb = self.getObservationProb(observation, pacPos, pos, self.getJailPosition())
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.beliefs:
            ...
            for newPos, prob in newPosDist.items():
                newBeliefs[newPos] += self.beliefs[pos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for pos in self.legalPositions:
            self.particles.extend([pos] * particles_per_position)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            obs_prob = self.getObservationProb(observation, gameState.getPacmanPosition(), particle, self.getJailPosition())
            tmp[particle] += obs_prob
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        num_positions = len(self.legalPositions)
        if self.numParticles > num_positions:
            self.numParticles = num_positions
        particles_per_position = self.numParticles // num_positions
        all_possible_positions = itertools.product(self.legalPositions, repeat=self.numGhosts)
        self.particles = random.sample(list(all_possible_positions), self.numParticles)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            observation_prob = 1
            for i, ghost_position in enumerate(particle):
                observation_prob *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), ghost_position, self.getJailPosition(i))
            weights[particle] += observation_prob
...
# One JointInference module is shared globally across instances of MarginalInference͏󠄂͏️͏󠄌͏󠄎͏︈͏︂͏︁
